...
// Host relay (docs/plans/host-relay-spec.md 8). Read-only: TRUE when our link
// to that player rides the host instead of a direct p2p link.
BOOL VPAPI vpPeerIsRelayed(IN VPSESSIONHANDLE pSesHdl, IN VPPLAYERID playerId);

// Kick the one-shot direct dial to a peer (lobby probe). No-op unless the relay
