...
class CPlayer : public CObject
{
  ...
  public:
    ...
    // BUILDINGS as a fraction of base -- 80 / 74 / 71 percent by tier, i.e. a diminishing
    // 20 / 26 / 29 percent reduction, or 1.25x / 1.35x / 1.41x effective building hit
    // points. Read straight off the research flags and applied at damage time in
    ...
    float GetBldgArmorMult( )
    {
        ...
        static const float afArmor[4] = { 1.00f, 0.80f, 0.74f, 0.71f };
        if ( iTier < 0 ) iTier = 0;
        if ( iTier > 3 ) iTier = 3;
