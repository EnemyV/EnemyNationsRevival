...
//
// Cross-platform MP sends the CNetCmd/CMsg structs in netcmd.h byte-for-byte
// over the socket. They are `#pragma pack(...,1)`, use only fixed-width members
// (DWORD/int/signed char + int-sized enums), no `long`/`time_t`/`size_t`, all
...
// Update only with a deliberate, lead+human-approved wire-format change.
//---------------------------------------------------------------------------
#include "stdafx.h"
#include "netcmd.h"
...
static_assert(sizeof(CNetLobbyRace)==140, "CNetLobbyRace");   // CNetCmd(12)+int(4)+CInitData(124), mirrors CNetReady
