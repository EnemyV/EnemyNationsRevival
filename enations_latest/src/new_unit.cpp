...
int CFarmBuilding::ForestMultAt( CHexCoord _hex, int iTyp, int iDir, int iScale )
{
    ...
    // get the number of forest hexes. Uses LumberBox so this yield scan and the Slash
    // and Burn slash scan CANNOT diverge -- see LumberBox above.
    int iRtn = 0;
    int ex, ey;
    LumberBox( _hex, ex, ey, pData, iDir );
...
void CVehicle::Serialize( CArchive& ar )
{
    ...
    if ( ar.IsStoring( ) )
    {
        ...
        // accidental policy. Note the probe's iTrue stays -1 here while iPos is N-1:
        // that gap is exactly what the [ROUTECUR] line below exists to measure.
        if ( ( m_pos == NULL ) && ( iOn > 0 ) )
            iPos = iOn - 1;
        ar << (WORD)iPos;
