...
void CUnit::AssignData( CUnitData const* pData )
{
    ...
    // Range and Attack R&D bonuses, in PARTS PER 10,000 of the unit's base value.
    //
    // Levels 0-3 are the original game's `base >> (4 - level)`. 625 / 1250 / 2500 / 5000 are
    // exactly 1/16, 1/8, 1/4, 1/2, and base*625 cannot overflow before the divide, so the
    // integer result is IDENTICAL to the old shift for every base value -- the stock ladder
    // (+6.25 / 12.5 / 25 / 50 percent) is unchanged.
    //
    // Level 4+ are the in-code tiers. The stock ladder DOUBLES its step every level (+6.25,
    // +12.5, +25 points), atk_4 takes one more +12.5 step to 62.5, and from there every tier
    // adds a FLAT +2 points -- the floor, not a halving, because the totals are what stack and
    // a geometric tail would have run the cap up far past the stock +50. Attack ends at level 8
    // (+70.5 percent of base, about +13.7 percent damage over the stock cap); range stops at
    // level 4. These are TOTALS, not per-tier additions: the level is a single lookup, so a
    // higher tier REPLACES the lower tier's bonus. A table replaces the shift chain here for
    // two reasons: `>> (4 - level)` shifts by ZERO at level 4 (a silent doubling) and is
    // undefined beyond it, and no shift can express a flat +2 point step.
    static const int aiRangeBonus[5]  = { 625, 1250, 2500, 5000, 6250 };
    static const int aiAttackBonus[9] = { 625, 1250, 2500, 5000, 6250, 6450, 6650, 6850, 7050 };
    {
        int rgB = GetData( )->_GetRange( );
        int rgL = GetOwner( )->GetRangeLevel( );
        if ( rgL < 0 ) rgL = 0;
        if ( rgL > 4 ) rgL = 4;
        m_iRange = rgB + ( ( rgB * aiRangeBonus[rgL] ) / 10000 );
    }
    {
        int atL = GetOwner( )->GetAttackLevel( );
        if ( atL < 0 ) atL = 0;
        if ( atL > 8 ) atL = 8;
        for ( int iInd = 0; iInd < CUnitData::num_attacks; iInd++ )
        {
            int atB = GetData( )->_GetAttack( iInd );
            m_iAttack[iInd] =
                (int)( 0.5f + GetOwner( )->GetAttackMult( ) *
                                  (float)( atB + ( ( atB * aiAttackBonus[atL] ) / 10000 ) ) );
        }
    }
    m_iDefense =
