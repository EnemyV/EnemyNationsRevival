...
void CRsrchArray::Open( )
{
    ...
    // The extended tiers 11-12 continue this one curve (see the block below).
    {
        ...
        for ( int iOn = 0; iOn < 10; iOn++ )
        {
            ...
            // Premium cost curve: KEEP DOUBLING every tier (no flat cap) — B * 2^(tier-1).
            // t1=B, t6=32B, t10=512B. Escalates past Fuel Efficiency (which caps at 32B),
            // so late speed tiers cost what their deep unlock warrants.
    ...
    // the fuel-efficiency line is exhausted by here so there is no fuel gate. Cost CONTINUES
    // the base line's pure-doubling premium curve (B * 2^(tier-1), no flat cap): tier 10 =
    // 512*B, so 11 = 1024*B and 12 = 2048*B. Appended at the END of the research enum
    // (vehicle_speed_11/12), chaining back to vehicle_speed_10 for tier 11.
    {
        ...
        for ( int iOn = 0; iOn < 2; iOn++ )
        {
            ...
            // Continue the pure-doubling premium curve: B * 2^(tier-1). Tier 10 = 512*B,
            // so tier 11 = 1024*B and tier 12 = 2048*B. Seamless doubling, no flat cap.
            int iTier = 11 + iOn;                                                // 11..12
            pRi->m_iPtsRequired      = iBase << ( iTier - 1 );
            pRi->m_iScenarioReq      = ElementAt( gas_turbine ).m_iScenarioReq;
            pRi->m_iNumBldgsRequired = 0;

            // Tier 11 chains the base line's top (vehicle_speed_10); tier 12 chains tier 11.
