...
=== Game data file (required)

The game data file `ENations.dat` (~516 MB) is *not* included in this repository
because of its size. Download it separately:

https://drive.google.com/file/d/16y40cxiC0dcmfB2vwZXU5VATqGD6fe_p/view?usp=sharing

Place `ENations.dat` in the working directory the game runs from (next to the
executable). The game looks for `.\ENations.dat` by default; the path is stored
in the `Game\DataFile` setting (Windows registry, or
`~/.config/enations/registry.ini` on Linux).

Prerequisites
----
ccache-4.8.1
VS2022 with prerequisites"
+Desktop dev w/ C++
+Game development with C++
x86 C++ 2015–2022 Redistributable 
----
...
generate project directory with cmake
I use the VS Developer Console for this
----
...
Open in VS 2022
----
open cmakeBuild folder
Launch Enations.sln
Select Release/Win32
Build wind22, vdmplay, then finally enations
Run enations.exe

optionally: set enations as your startup project
----