...
void handle_command(const std::string& line, en_socket_t conn) {
    ...
    if (strcmp(cmd, "shot") == 0 || strcmp(cmd, "shotid") == 0) {
        ...
        en_send(conn, out.c_str(), out.size());
        return;
    } else if (strcmp(cmd, "testcorridor") == 0) {
        int x = -1, y = -1, length = 0;
        char extra;
        if (sscanf(line.c_str(), "%*s %d %d %d %c", &x, &y, &length, &extra) != 3 ||
            length < 41 || length > 80) {
            const char* usage = "err usage: testcorridor <x> <y> <length 41..80>\n";
            en_send(conn, usage, strlen(usage)); return;
        }
        g_roadX1=x; g_roadY1=y; g_roadX2=length; g_roadY2=0;
...
void EnHarness_Service() {
    ...
    if (g_roadPending.exchange(false)) {
        ...
        if (g_testCorridor.load())
            HarnessTestCorridor(g_roadX1.load(), g_roadY1.load(), g_roadX2.load(), out);
        else
